...
std::string  load_elf_64bit(std::ifstream&  elf,
                            file_props_ptr  elf_props,
                            load_props_elf&  load_props)
{
    ASSUMPTION( elf.is_open() );
    ASSUMPTION( elf_props->num_address_bits() == 64U );

    static std::map<std::string,abi_virtual_address_selector> const  abi_loaders_choose_address{
            {abi_loaders::ld_linux_x86_64_so_2(),ld_linux_x86_64_so_2::choose_address_for_section},
            {abi_loaders::ld_linux_uClibc_so_0(),ld_linux_x86_64_so_2::choose_address_for_section}
            };
    static std::map<std::string,abi_pltgot_section_split_func> const  abi_loaders_split_section_by_pltgot{
            {abi_loaders::ld_linux_x86_64_so_2(),ld_linux_x86_64_so_2::split_section_by_pltgot},
            {abi_loaders::ld_linux_uClibc_so_0(),ld_linux_x86_64_so_2::split_section_by_pltgot}
            };
    static std::map<std::string,abi_special_symbol_hash_table_parser> const  abi_loaders_special_symbol_hash_table_parser{
            {abi_loaders::ld_linux_x86_64_so_2(),ld_linux_x86_64_so_2::special_symbol_hash_table_parser},
            {abi_loaders::ld_linux_uClibc_so_0(),ld_linux_x86_64_so_2::special_symbol_hash_table_parser}
            };
    static std::map<std::string,abi_relocations_fn> const  abi_loaders_perform_relocations{
            {abi_loaders::ld_linux_x86_64_so_2(),ld_linux_x86_64_so_2::perform_relocations},
            {abi_loaders::ld_linux_uClibc_so_0(),ld_linux_x86_64_so_2::perform_relocations}
            };

    bool const  is_it_load_of_root_binary = load_props.root_file() == elf_props->path();

    if (fileutl::file_size(elf_props->path()) < 64ULL)
        LOAD_ELF_WARNING("The file is too small to contain even the ELF header.");
    ...
    uint64_t  entry_point = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());

    // Load of sections
    uint64_t const  program_header_offset = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());

    fileutl::skip_bytes(elf,8U); // We do not use section header offset.
...

    {
        ...
        if (elf_header_size != 64U)
            LOAD_ELF_WARNING("[Header offset 0x34] Wrong size of the ELF header 64bit (have to be 64 bytes). "
                             "We ignore it.");
    }
    ...
    uint64_t  symbol_hash_table_type = 0ULL;

    bool  uses_load_flags = false;
    uint64_t  load_flags = 0U;

    bool  tls_used = false;
    uint64_t  tls_file_offset = 0ULL;
    uint64_t  tls_size_in_file = 0ULL;
    uint64_t  tls_template_raw_address = 0ULL;
    uint64_t  tls_template_size_in_memory = 0ULL;
    uint64_t  tls_alingment = 0ULL;
    uint32_t  tls_flags = 0ULL;
    ...
    for (uint32_t  i = 0U; i < program_header_table_num_entries; ++i)
    {
        ...
        uint64_t const  offset = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
        uint64_t const  virtual_address = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());

        fileutl::skip_bytes(elf,8U); // We do not use physical address.

        uint64_t const  size_in_file = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
        uint64_t const  size_in_memory = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
        uint64_t const  align = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());

        switch (type)
        {
        // We consider only the following sections:
        case 1U:    // DT_LOAD
        case 2U:    // DT_DYNAMIC
        case 3U:    // DT_INTERP
        case 7U:    // DT_TLS
        ...
        case 2U: // We detected DYNAMIC dynamic liking segment.
            {
                ...
                while ((uint64_t)elf.tellg() < offset + size_in_file)
                    switch (fileutl::read_bytes_to_int64_t(elf,8U,elf_props->is_in_big_endian()))
                    {
                    case 1ULL: // We detected NEEDED entry
                        string_table_offsets.push_back(fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian()));
                        break;
                    case 2ULL: // We detected PLTRELSZ entry
                        plt_relocations_table_size = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 3ULL: // We detected PLTGOT entry
                        pltgot_address = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 4ULL: // We detected symbol HASH table entry
                        symbol_hash_table_address = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        symbol_hash_table_type = 4ULL;
                        break;
                    case 5ULL: // We detected STRTAB entry
                        string_table_file_address = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 6ULL: // We detected SYMTAB entry
                        symbol_table_address = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 7ULL: // We detected RELA entry
                        got_relocations_table_address = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 8ULL: // We detected RELASZ entry
                        got_relocations_table_size = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 9ULL: // We detected RELAENT entry
                        got_relocations_table_entry_size = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 10ULL: // We detected STRSZ entry
                        string_table_size = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 11ULL: // We detected SYMENT entry
                        symbol_table_entry_size = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 12ULL: // We detected INIT entry
                        init_fn_address = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 13ULL: // We detected FINI entry
                        fini_fn_address = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 17ULL: // We detected REL entry
                        return "The x86-64bit ELF binary contains '.rela.dyn' REL relocation table type (but RELA is expected).";
                    case 20ULL: // We detected PLTREL entry
                        plt_relocations_table_type = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 23ULL: // We detected JMPREL entry
                        plt_relocations_table_address = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 25ULL: // We detected INIT_ARRAY entry
                        init_fn_array_address = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 26ULL: // We detected FINI_ARRAY entry
                        fini_fn_array_address = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 27ULL: // We detected INIT_ARRAYSZ entry
                        init_fn_array_size = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 28ULL: // We detected FINI_ARRAYSZ entry
                        fini_fn_array_size = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 30ULL: // We detected FLAGS
                        uses_load_flags = true;
                        load_flags = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 32ULL: // We detected PREINIT_ARRAY entry
                        preinit_fn_array_address = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 33ULL: // We detected PREINIT_ARRAYSZ entry
                        preinit_fn_array_size = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                        break;
                    case 0x6ffffef5ULL: // We detected DT_GNU_HASH entry
                        if (symbol_hash_table_address == 0ULL)
                        {
                            symbol_hash_table_address = fileutl::read_bytes_to_uint64_t(elf,8U,elf_props->is_in_big_endian());
                            symbol_hash_table_type = 0x6ffffef5ULL;
                        }
                        else
                        {
                            fileutl::skip_bytes(elf,8U);
                            if (symbol_hash_table_type != 4ULL)
        ...
        case 7U: // We detected TLS (thread-local storage).
            {
                tls_used = true;
                tls_file_offset = offset;
                tls_size_in_file = size_in_file;
                tls_template_raw_address = virtual_address;
                tls_template_size_in_memory = size_in_memory;
                tls_alingment = align;
                tls_flags = flags;
                break;
            }
    ...
    if (is_it_load_of_root_binary)
    {
        if (elf_props->property(file_properties::abi_loader()) == abi_loaders::NONE())
        {
                ...
                load_props.platform()->abi() == abi_t::LINUX )
            {
                ...
                abi_loader_name = abi_loaders::ld_linux_x86_64_so_2();
            }
    ...
    if (preinit_fn_array_address != 0ULL && is_it_load_of_root_binary)
        for (address  preinit_table_ptr = load_props.fixed_address_for(elf_props->path(),preinit_fn_array_address),
                ...
                preinit_table_ptr += sizeof(address))
            load_props.add_init_function(
                        elf_props->path(),
                        load_props.fixed_address_for(
                                elf_props->path(),
                                read_address_64bit(preinit_table_ptr,load_props.sections_table())
                                )
                        );
    ...
    if (init_fn_array_address != 0ULL)
        for (address  init_table_ptr = load_props.fixed_address_for(elf_props->path(),init_fn_array_address),
                ...
                init_table_ptr += sizeof(address))
            load_props.add_init_function(
                        elf_props->path(),
                        load_props.fixed_address_for(
                                elf_props->path(),
                                read_address_64bit(init_table_ptr,load_props.sections_table())
                                )
                        );
    if (fini_fn_array_address != 0ULL)
        for (address  fini_table_ptr = load_props.fixed_address_for(elf_props->path(),fini_fn_array_address),
                ...
                fini_table_ptr += sizeof(address))
            load_props.add_fini_function(
                        elf_props->path(),
                        load_props.fixed_address_for(
                                elf_props->path(),
                                read_address_64bit(fini_table_ptr,load_props.sections_table())
                                )
                        );
    ...
    if (symbol_table_entry_size == 3ULL * sizeof(uint64_t))
    {
        ...
        if (symbol_hash_table_type == 4ULL) // DT_HASH  (the one of the ELF specification)
        {
            ...
            address const  chains_end  = chains_begin + num_chains * 4ULL;
        ...
        if (string_table_file_address > symbol_table_address
                && (string_table_file_address - symbol_table_address) % symbol_table_entry_size == 0ULL)
        {
            symbol_table_begin_index = 0ULL;
            symbol_table_end_index = (string_table_file_address - symbol_table_address) / symbol_table_entry_size;
        }

        for (uint64_t  i = symbol_table_begin_index; i < symbol_table_end_index; ++i)
        {
            ...
            uint8_t const  symbol_info = read_uint8_t(ptr + 4ULL,load_props.sections_table());

            // We do not read the following (commented) elements, since we do not need them.
            //uint8_t const  symbol_other = read_uint8_t(ptr + 5ULL,load_props.sections_table());
            //uint16_t const  symbol_shndx = read_uint16_t(ptr + 6ULL,load_props.sections_table());

            uint64_t const  symbol_value = read_uint64_t(ptr + 8ULL,load_props.sections_table());
            uint64_t const symbol_size = read_uint64_t(ptr + 16ULL,load_props.sections_table());
            ...
            switch (symbol_type)
            {
            case 1U /*STT_OBJECT*/:symbol_type_name = relocation_type::DATA(); break;
            case 2U /*STT_FUNC*/: symbol_type_name = relocation_type::FUNCTION(); break;
            case 5U /*STT_COMMON*/: symbol_type_name = relocation_type::COMMON(); break;
            case 6U /*STT_TLS*/: symbol_type_name = relocation_type::TLS(); break;
    ...
    if (tls_used)
    {
        address const  tls_template_fixed_address = load_props.fixed_address_for(elf_props->path(),tls_template_raw_address);

        load_props.add_special_section(elf_props->path(),special_section_name::thread_local_storage(),
                                       std::make_shared<special_section::elf_tls const>(
                                            tls_template_fixed_address,
                                            tls_template_fixed_address + tls_size_in_file,
                                            tls_template_fixed_address + tls_template_size_in_memory,
                                            tls_file_offset,
                                            tls_alingment,
                                            tls_flags,
                                            uses_load_flags ? ((load_flags & 0x10ULL) != 0ULL) : false
                                            ));
    }
