...
std::string  load_pe_64bit(std::ifstream&  pe, file_props_ptr  pe_props,
                           coff_standard_header_info const&  coff_info,
                           load_props_pe&  load_props)
{
    ASSUMPTION( pe.is_open() );
    ASSUMPTION( pe_props->num_address_bits() == 64U );
    ...
    {
        image_base = fileutl::read_bytes_to_uint64_t(pe,8U,pe_props->is_in_big_endian());
        ...
        set_file_property(pe_props->property_map(),file_properties::max_stack_size(),
                          to_string(fileutl::read_bytes_to_uint64_t(pe,8U,pe_props->is_in_big_endian())));

        fileutl::skip_bytes(pe,8U); // We do not use initial (commit) stack size.

        set_file_property(pe_props->property_map(),file_properties::max_local_heap_size(),
                          to_string(fileutl::read_bytes_to_uint64_t(pe,8U,pe_props->is_in_big_endian())));

        fileutl::skip_bytes(pe,8U); // We do not use initial (commit) local heap size.
    ...
    uint64_t  exceptions_table_address = 0ULL;
    uint64_t  exceptions_table_size = 0ULL;
...

    {
        for (uint64_t  num_entries = 0ULL; num_entries < num_data_directory_entries; )
        {
            ...
            exceptions_table_address = fileutl::read_bytes_to_uint64_t(pe,4U,pe_props->is_in_big_endian());
            exceptions_table_size = fileutl::read_bytes_to_uint64_t(pe,4U,pe_props->is_in_big_endian());
            if (++num_entries == num_data_directory_entries)
                break;
    ...
    for (uint32_t  i = 0U; i < coff_info.number_of_sections(); ++i)
    {
        ...
        if (aligned_size_in_file % file_alignment != 0ULL)
            return "The size-in-file of the section at index " + to_string(i) + " is not properly aligned.";
             ...
             block_ptr < block_end; )
        {
                 ...
                 ptr += 2U)
            {
                ...
                case 1: // IMAGE_REL_BASED_HIGH
                    ...
                    return "Base relocation of the type 1 is NOT IMPLEMENTED YET!";
                    break;
                case 2: // IMAGE_REL_BASED_LOW
                    ...
                    return "Base relocation of the type 2 is NOT IMPLEMENTED YET!";
                    break;
                case 3: // IMAGE_REL_BASED_HIGHLOW
                        //   The base relocation applies all 32 bits of the difference to the
                        //   32-bit field at offset.
                    return "Base relocation of the type 3 is NOT IMPLEMENTED YET!";
                    break;
    ...
    if (import_table_size > 0ULL)
    {
        ...
        for (address  import_dir_ptr = fixed_address + import_table_address; true; import_dir_ptr += 20ULL)
        {
            ...
            for (uint64_t  i = 0ULL; true; ++i)
            {
                if (!find_section(lookup_table_begin + i * 8ULL,load_props.sections_table()).operator bool())
                {
                ...
                uint64_t const  lookup_value = read_uint64_t(lookup_table_begin + i * 8ULL,load_props.sections_table());
                if (lookup_value == 0ULL)
                    break;

                bool const  do_import_via_ordinal = (lookup_value & 0x8000000000000000ULL) != 0ULL;
                ...
                std::string  imported_symbol_name;
                {
                    ...
                    {
                        imported_symbol_ordinal = (uint32_t)(lookup_value & 0xffffULL);
                        imported_symbol_adr = load_props.symbol_address(lib_file_name,imported_symbol_ordinal);
                    }
                    else
                    {
                        address const  hint_name_table_element_ptr = fixed_address + (lookup_value & 0xffffffffULL);
                ...
                {
                    write_address(address_table_begin + i * 8ULL,imported_symbol_adr,load_props.sections_table());
                    load_props.add_performed_relocation({
                            address_table_begin + i * 8ULL,
                            8ULL,
    ...
    if (load_config_table_size > 0ULL)
    {
                        ...
                        });
        {
            ...
            if (load_config_table_size < 96ULL)
                LOAD_PE_WARNING("The load config structure is too short.")
            else if (load_props.platform()->architecture() == architecture_t::X86_64 && load_config_table_size < 112ULL)
            ...
            else
            {
                ...
                uint64_t const  decommit_block_thresold = read_uint64_t(ptr + 24ULL,load_props.sections_table());
                uint64_t const  decommit_free_thresold = read_uint64_t(ptr + 32ULL,load_props.sections_table());
                address const  lock_prefix_table =
                        load_props.platform()->architecture() == architecture_t::X86_64 ?
                                read_address_64bit(ptr + 40ULL,load_props.sections_table())
                                : 0ULL;
                uint64_t const  max_alloc_size = read_uint64_t(ptr + 48ULL,load_props.sections_table());
                uint64_t const  memory_thresold = read_uint64_t(ptr + 56ULL,load_props.sections_table());
                uint64_t const  process_affinity_mask = read_uint64_t(ptr + 64ULL,load_props.sections_table());
                uint32_t const  process_heap_flags = read_uint32_t(ptr + 72ULL,load_props.sections_table());
                uint16_t const  service_pack_version = read_uint16_t(ptr + 76ULL,load_props.sections_table());
                address const  security_cookie = read_uint64_t(ptr + 88ULL,load_props.sections_table());
                std::vector<address>  structured_exception_handlers;
                {
                    address const  seh_begin  = read_address_64bit(ptr + 96ULL,load_props.sections_table());
                    uint64_t const  seh_count  = read_uint64_t(ptr + 104ULL,load_props.sections_table());
                    for (address  seh_ptr = seh_begin; seh_ptr < seh_begin + seh_count * 8ULL; seh_ptr += 8ULL)
                        if (!find_section(seh_ptr,load_props.sections_table()).operator bool())
                            ...
                            structured_exception_handlers.push_back(
                                        fixed_address + read_address_64bit(seh_ptr,load_props.sections_table())
                                        );
                }
    ...
    if (exceptions_table_size > 0ULL)
    {
        special_section_properties_ptr  section_props;
        if (load_props.platform()->architecture() == architecture_t::X86_64)
        {
            std::vector< std::tuple<address,address,address> >  records;
            for (address  ptr = fixed_address + exceptions_table_address,
                          end = fixed_address + exceptions_table_address + exceptions_table_size;
                 ptr < end; ptr += 12ULL)
            {
                address const  begin_addr = fixed_address + read_uint32_t(ptr + 0ULL,load_props.sections_table());
                address const  end_addr = fixed_address + read_uint32_t(ptr + 4ULL,load_props.sections_table());
                address const  unwind_info_addr = fixed_address + read_uint32_t(ptr + 8ULL,load_props.sections_table());
                records.push_back(std::make_tuple(begin_addr,end_addr,unwind_info_addr));
            }
            section_props = special_section_properties_ptr( new special_section::exceptions{
                                    fixed_address + exceptions_table_address,
                                    fixed_address + exceptions_table_address + exceptions_table_size,
                                    records
                                    });
        }
        else
            return "The load of the special section 'exceptions' is not implemented the the file's target platform.";

        INVARIANT(section_props.operator bool());
        load_props.add_special_section(pe_props->path(),special_section_name::exceptions(),section_props);
    }
