                           ...
                           load_props_pe&  load_props)
{
    ...
    {
        image_base = read_bytes_to_uint64_t(pe,8U,pe_props->is_in_big_endian());
        ...
        set_file_property(pe_props->property_map(),file_properties::max_stack_size(),
                          to_string(read_bytes_to_uint64_t(pe,8U,pe_props->is_in_big_endian())));

        skip_bytes(pe,8U); // We do not use initial (commit) stack size.

        set_file_property(pe_props->property_map(),file_properties::max_local_heap_size(),
                          to_string(read_bytes_to_uint64_t(pe,8U,pe_props->is_in_big_endian())));

        skip_bytes(pe,8U); // We do not use initial (commit) local heap size.
...

    {
        for (uint64_t  num_entries = 0ULL; num_entries < num_data_directory_entries; )
        {
            ...
            exceptions_table_address = read_bytes_to_uint64_t(pe,4U,pe_props->is_in_big_endian());
            exceptions_table_size = read_bytes_to_uint64_t(pe,4U,pe_props->is_in_big_endian());
            if (++num_entries == num_data_directory_entries)
                break;
